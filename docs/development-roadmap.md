...
- `plan-046-schema-validation-and-migration` 的第一片策略是“校验优先、迁移克制”：`from_dict()` 继续允许 legacy readable records 使用默认值读取；`abh doctor` 负责报告 missing required field、unknown field、deprecated field、missing schema_version 和 unsupported schema_version。Deprecated 字段是 schema 中显式列出的旧字段，当前第一例是 plan record 的 `prepared_at`；doctor 会报告但不会自动迁移或删除。未知字段先作为 doctor issue 暴露，不自动删除；历史对象不批量重写，除非后续 migration plan 明确要求。
...
- `plan-049-commitment-phase-state`（已完成）
- `plan-050-audit-semantic-conservation`（已完成）
- `plan-051-owner-doc-stable-commitments`（已完成）
- `plan-052-post-close-freshness-semantics`（已完成）
...
- `plan-047-verification-runner-trust-policy`（已完成）
- `plan-048-test-suite-domain-split`（已完成）
...
- 提供 GitHub Actions 模板：PR 中自动跑 `abh doctor`、测试和漂移检查。Stage 7 CI drift boundary: `plan-053-ci-templates` 将这里的漂移检查收敛为 roadmap consistency, whitespace drift, and read-only health posture；当前 CI template does not run standalone `abh drift analyze`，也不把历史 health pressure 自动升级为团队策略或 release blocker。
...
- `plan-053-ci-templates`（由 `stage7.ci-templates` materialize，已关闭）
- `plan-054-multi-repo-sharing`（由 `stage7.multi-repo-sharing` materialize，blocked/deferred；独立审计确认缺少多仓库共享 R-flow 实现证据，后续需另开实现计划）
- `plan-055-abh-workflow-skill`（由 `adoption.abh-workflow-skill` materialize，已关闭；把现有 ABH CLI 编排成傻瓜式 Codex skill）
...
| 阶段 6：漂移与记忆质量提升 | `plan-004-sprint-4-route-drift`, `plan-007-sprint-7-dogfood`, `plan-039-quality-signal-model`, `plan-040-drift-quality`, `plan-041-memory-index`, `plan-042-project-health-report`, `plan-044-command-contract-runtime-registry`, `plan-045-repository-write-transaction-boundary`, `plan-046-schema-validation-and-migration`, `plan-047-verification-runner-trust-policy`, `plan-048-test-suite-domain-split`, `plan-049-commitment-phase-state`, `plan-050-audit-semantic-conservation`, `plan-051-owner-doc-stable-commitments`, `plan-052-post-close-freshness-semantics` | 关键词 drift、route 注入活跃计划和记忆；`plan-039` 定义 product-quality-first / agent-navigation-second 的质量信号模型；`plan-040` 提升 drift finding 质量；`plan-041` 已把 memory 提升为带 tags、status、typed relationships 和 supersession 的可复用质量知识；`plan-042` 已交付只读语义压力报告；command contract runtime registry、repository write boundary、schema validation/migration、verification runner trust policy、test suite domain split、commitment phase state、audit semantic conservation、owner-doc stable commitments 和 post-close freshness semantics 已关闭 | `plan-043-plan-reference-set` 已 deferred |
| 阶段 7：团队可用与生态集成 | `plan-007-zero-dep-install`, `plan-053-ci-templates`, `plan-054-multi-repo-sharing`, `plan-055-abh-workflow-skill` | uvx/uv tool install 降低接入门槛；`plan-053-ci-templates` 已把 PR CI 模板、复用说明和本地优先验证链路落到 Stage 7 接入面；`plan-054-multi-repo-sharing` 已完成计划定义但因缺少功能 R-flow 证据 blocked/deferred；`plan-055-abh-workflow-skill` 已关闭，交付现有 ABH CLI 工作流的傻瓜式 skill 封装 | 多仓库实现、团队策略、发布自动化 |

## 7. 下一批推荐计划

本节只列下一批仍可切分执行的计划。已关闭的 `plan-012-agent-protocol-foundation`、`plan-013-json-output-and-errors`、`plan-014-readonly-mcp-server`、`plan-015-controlled-mcp-write-tools`、阶段 3 的 `plan-016` 至 `plan-025`、release-prep 的 `plan-026-v0-3-release-prep`、Stage 4 的 `plan-027-stage-4-attractor-entry-plan` 至 `plan-036-quickstart-recipes-and-distribution`，Stage 5 的 `plan-037-audit-prompt-bundle` 和 `plan-038-independent-audit-gate`，Stage 6 的 `plan-039-quality-signal-model`、`plan-040-drift-quality`、`plan-041-memory-index`、`plan-042-project-health-report`、`plan-044-command-contract-runtime-registry`、`plan-045-repository-write-transaction-boundary`、`plan-046-schema-validation-and-migration`、`plan-047-verification-runner-trust-policy`、`plan-048-test-suite-domain-split`、`plan-049-commitment-phase-state`、`plan-050-audit-semantic-conservation`、`plan-051-owner-doc-stable-commitments`、`plan-052-post-close-freshness-semantics`，以及 Stage 7 的 `plan-053-ci-templates` 和 adoption 切片 `plan-055-abh-workflow-skill` 已归入第 3 章历史执行线与第 6 章阶段映射。`plan-043-plan-reference-set` 已重新排期为 blocked/deferred；`plan-054-multi-repo-sharing` 已 blocked/deferred，不再作为当前实现焦点；当前无 open plan，下一步以 `abh next --json` 为准。未来未 materialize 事项的事实来源是 `.abh/roadmap.json`，文档中不再为未 materialize 项预写具体 plan 编号。
