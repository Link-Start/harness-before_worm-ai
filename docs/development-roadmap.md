...
- `plan-057-codex-repo-toggle`（进行中；为 Codex 桌面版提供仓库级受管 `.codex/config.toml` toggle）
...
| 阶段 7：团队可用与生态集成 | `plan-007-zero-dep-install`, `plan-053-ci-templates`, `plan-054-multi-repo-sharing`, `plan-055-abh-workflow-skill`, `plan-057-codex-repo-toggle` | uvx/uv tool install 降低接入门槛；`plan-053-ci-templates` 已把 PR CI 模板、复用说明和本地优先验证链路落到 Stage 7 接入面；`plan-054-multi-repo-sharing` 已完成计划定义但因缺少功能 R-flow 证据 blocked/deferred；`plan-055-abh-workflow-skill` 已关闭，交付现有 ABH CLI 工作流的傻瓜式 skill 封装；`plan-057-codex-repo-toggle` 正在补齐 Codex 桌面版仓库级 ABH toggle | 多仓库实现、团队策略、发布自动化 |

## 7. 下一批推荐计划

本节只列下一批仍可切分执行的计划。已关闭的 `plan-012-agent-protocol-foundation`、`plan-013-json-output-and-errors`、`plan-014-readonly-mcp-server`、`plan-015-controlled-mcp-write-tools`、阶段 3 的 `plan-016` 至 `plan-025`、release-prep 的 `plan-026-v0-3-release-prep`、Stage 4 的 `plan-027-stage-4-attractor-entry-plan` 至 `plan-036-quickstart-recipes-and-distribution`，Stage 5 的 `plan-037-audit-prompt-bundle` 和 `plan-038-independent-audit-gate`，Stage 6 的 `plan-039-quality-signal-model`、`plan-040-drift-quality`、`plan-041-memory-index`、`plan-042-project-health-report`、`plan-044-command-contract-runtime-registry`、`plan-045-repository-write-transaction-boundary`、`plan-046-schema-validation-and-migration`、`plan-047-verification-runner-trust-policy`、`plan-048-test-suite-domain-split`、`plan-049-commitment-phase-state`、`plan-050-audit-semantic-conservation`、`plan-051-owner-doc-stable-commitments`、`plan-052-post-close-freshness-semantics`，以及 Stage 7 的 `plan-053-ci-templates` 和 adoption 切片 `plan-055-abh-workflow-skill` 已归入第 3 章历史执行线与第 6 章阶段映射。`plan-043-plan-reference-set` 已重新排期为 blocked/deferred；`plan-054-multi-repo-sharing` 已 blocked/deferred，不再作为当前实现焦点；`plan-057-codex-repo-toggle` 是当前 open plan。未来未 materialize 事项的事实来源是 `.abh/roadmap.json`，文档中不再为未 materialize 项预写具体 plan 编号。
