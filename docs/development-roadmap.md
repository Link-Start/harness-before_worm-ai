...
当前进行计划：无 open plan。`abh next --json` 当前推荐 materialize 下一条 queued roadmap item：`stage6.drift-quality`。
...
本节只列下一批仍可切分执行的计划。已关闭的 `plan-012-agent-protocol-foundation`、`plan-013-json-output-and-errors`、`plan-014-readonly-mcp-server`、`plan-015-controlled-mcp-write-tools`、阶段 3 的 `plan-016` 至 `plan-025`、release-prep 的 `plan-026-v0-3-release-prep`、Stage 4 的 `plan-027-stage-4-attractor-entry-plan` 至 `plan-036-quickstart-recipes-and-distribution`，以及 Stage 5 的 `plan-037-audit-prompt-bundle` 和 `plan-038-independent-audit-gate` 已归入第 3 章历史执行线与第 6 章阶段映射。下一条 queued roadmap item 是 `stage6.drift-quality`。未来事项的事实来源是 `.abh/roadmap.json`，文档中不再为未 materialize 项预写具体 plan 编号。
